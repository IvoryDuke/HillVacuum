...
impl<'a, T> SliceTripletIter<'a, T>
{
    ...
    /// Returns an iterator that returns the slice indexes of the elements along with the pair
...
/// 0 is grouped with the highest and second to highest elements, and 1 is grouped with 0 and
/// the highest element.
#[must_use]
...
/// A slice iterator that returns the indexes of the elements along with the elements
/// themselves.
    ...
    I: Sized + Iterator + ExactSizeIterator
{
    ...
    /// Sets the iterator `factory` to `Self::unfiltered_iteration` once the index has been
    /// skipped.
    #[inline]
    #[must_use]
    fn filtered_iteration(&mut self) -> Option<I::Item>
