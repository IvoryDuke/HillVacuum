...
impl<'b> TextureInterface for MovingTextureSettings<'b>
{
    ...
    #[inline]
    fn draw_scroll_x(&self, elapsed_time: f32) -> f32 { self.texture.draw_scroll_x(elapsed_time) }

    #[inline]
    fn draw_scroll_y(&self, elapsed_time: f32) -> f32 { self.texture.draw_scroll_y(elapsed_time) }
    ...
    #[inline]
    fn sprite_hull(&self, center: Vec2) -> Hull { self.texture.sprite_hull(center + self.delta) }
...
impl ConvexPolygon
{
    ...
    fn valid(&self) -> bool
    {
        if !self.center.around_equal(&vxs_center(self.vertexes())) ||
    ...
    #[inline]
    #[must_use]
    pub(in crate::map::brush) fn take_texture_settings(self) -> Option<TextureSettings>
    {
        self.texture
    }
    ...
    ) -> bool
    {
        ...
        {
            Some(tex_set) => tex_set.check_texture_change(drawing_resources, texture, self.center),
            None => true
        }
    }
