...
{
    #[inline]
    pub fn draw(&mut self, bundle: &mut StateUpdateBundle, tools_buttons: &ToolsButtons)
    {
        #[inline]
        fn show(ui: &mut egui::Ui, tools_buttons: &ToolsButtons)
        {
            ...
            manual_section!(
                ...
                (
                    ...
                    "Textures must be placed in the assets/textures/ folder to be loaded.\nThe \
                     ...
                     Options menu.\n\nDefault textures animation can be exported and imported \
                     between map files. The file extension of the animations files is .anms."
                )
            );
            ...
            manual_section!(
                ...
                (
                    ...
                    "If there is a non-selected vertex beneath the cursor, it will be exclusively \
                     ...
                     released all selected vertexes they will be deselected."
                ),
                (
                    "Shift + Left mouse",
            ...
            manual_section!(
                ...
                (
                    "Snap Tool key",
                    "If the Vertex Tool is being used, all selected vertexes are snapped to the \
                     grid.\nIf the Side Tool is being used the selected sides are \
                     snapped.\nFinally, if any other tool is being used, all the vertexes of the \
                     selected brushes are snapped."
                ),
                (
                    "Alt + Snap Tool key",
                    "Quick snap: snaps the vertexes of the selected brushes to a two-units size \
                     grid."
                )
            );
        ...
        if key_inputs.just_pressed(HardcodedActions::ToggleManual.key())
        {
            self.0.open();
        }
        else if !self.0.is_open()
        {
            return;
        }
